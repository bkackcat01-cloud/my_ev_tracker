...
with tab_locations:
    if df.empty or df["Location"].dropna().empty:
        ...
        top_locations = df[df["Location"].notna() & (df["Location"] != "")].groupby("Location")["Total Cost"].sum().sort_values(ascending=False).head(5).reset_index()
        fig_loc = px.bar(top_locations, x="Location", y="Total Cost", text="Total Cost", title="Top 5 Locations by Spending")
